...
class RosApp(App):
    def __init__(self):
        ...
        self.task_status_sub = rospy.Subscriber("/task_status", Int32, self.task_status_callback)
        self.current_cluster_index = 0
        self.wait_for_task_to_start = False
        self.wait_for_task_to_finish = False
        self.cluster_paths = []  # 이 리스트는 각 컨벡스 헐에 대한 strand_posearray들을 저장합니다.
    ...
    def task_status_callback(self, msg):
        ...
        if self.wait_for_task_to_start and self.task_status == 1:
            self.wait_for_task_to_start = False
            self.wait_for_task_to_finish = True
        elif self.wait_for_task_to_finish and self.task_status == 0:
            self.wait_for_task_to_finish = False
            self.current_cluster_index += 1
            if self.current_cluster_index >= len(self.cluster_paths):
                self.current_cluster_index = 0  # 모든 클러스터가 완료되면 인덱스를 초기화합니다.

    def publish_next_cluster_path(self):
        if len(self.cluster_paths) > 0:
            # /task_status 토픽으로부터 메시지를 기다립니다.
            task_status = rospy.wait_for_message("/task_status", Int32, timeout=None)
            if task_status.data == 1:  # 작업 시작을 나타내는 메시지를 기다립니다.
                # 다음 클러스터의 경로를 발행합니다.
                strand_posearray = self.cluster_paths[self.current_cluster_index]
                self.strand_path_pub.publish(strand_posearray)

                # 작업 완료를 나타내는 메시지를 기다립니다.
                task_status = rospy.wait_for_message("/task_status", Int32, timeout=None)
                if task_status.data == 0:
                    self.current_cluster_index += 1
                    if self.current_cluster_index >= len(self.cluster_paths):
                        self.current_cluster_index = 0  # 모든 클러스터가 완료되면 인덱스를 초기화합니다.
    ...
    def main(self):             #
        ...
        while not rospy.is_shutdown():
            if self.cv_image is not None and self.cv_depth is not None:
                ...
                if len(hulls) > 0 and not self.wait_for_task_to_start and not self.wait_for_task_to_finish:
                    self.cluster_paths = []  # 클러스터 경로를 재설정합니다.
                    for i, hull in enumerate(hulls):
                        ...
                        if len(hull_3d) > 0:
                            ...
                            if estimated_plane is not None and plane_inliers is not None:
                                ...
                                self.cluster_paths.append(strand_posearray)

                    # 첫 번째 클러스터의 strand_posearray를 반복해서 발행합니다.
                    if len(self.cluster_paths) > 0:
                        print("waiting for combing...")
                        while self.task_status != 1:  # task_status가 1이 될 때까지 기다립니다.
                            self.strand_path_pub.publish(self.cluster_paths[0])
                            rospy.sleep(0.1)  # 너무 빠른 반복을 피하기 위한 작은 지연

                        # 이제 task_status가 1입니다. 0이 될 때까지 기다립니다.
                        while self.task_status != 0:
                            rospy.sleep(0.1)  # 상태 변경 대기
                        print("combing is finished!")

                        # task_status가 0이 되면 다음 클러스터로 넘어갑니다.
                        for strand_posearray in self.cluster_paths[1:]:
                            self.strand_path_pub.publish(strand_posearray)
                            print("waiting for combing...")
                            while self.task_status != 0:
                                rospy.sleep(0.1)  # 각 클러스터에 대해 완료될 때까지 기다립니다.
